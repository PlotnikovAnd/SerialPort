...
int main(int argc, char *argv[])
{
    ...
    auto printItem = [&](const auto & item) { qDebug() << item.portName(); };
    auto ports = QSerialPortInfo::availablePorts();

    std::for_each(ports.cbegin(), ports.cend(), printItem);


    // for(QSerialPortInfo& item : QSerialPortInfo::availablePorts()) {
    ...
    port->setPortName(QLatin1String(TEST_PORT_NAME));
    ...
    qDebug() << "RX: " << QString(QLatin1String(response.toHex()));
